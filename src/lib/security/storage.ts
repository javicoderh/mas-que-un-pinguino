...
const historicalFirestoreSignatureBaseline = {
  count: 5480,
  chileanCount: 5453,
  foreignCount: 27
} as const;
...
function mergeHistoricalBaseline(counter: SignatureCounterBreakdown): SignatureCounterBreakdown {
  return {
    count: historicalFirestoreSignatureBaseline.count + Number(counter.count ?? 0),
    chileanCount: historicalFirestoreSignatureBaseline.chileanCount + Number(counter.chileanCount ?? 0),
    foreignCount: historicalFirestoreSignatureBaseline.foreignCount + Number(counter.foreignCount ?? 0)
  };
}
